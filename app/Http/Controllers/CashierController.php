    ...
    public function showSession(CashierSession $session)
{
    ...
    public function recordWithdrawal(Request $request, CashierSession $session)
    {
        ...
        $withdrawal->save();
        //historiser l'action
        $this->historiser("session {$session->id} Retrait de caisse enregistré: 'amount' => {$validated['amount']}, 'reason' => {$validated['reason']}, 'withdrawn_by' => {$validated['withdrawn_by']}", 'retrait_caisse_enregistre');
        return redirect()->route('cashier.session', $session->id)
