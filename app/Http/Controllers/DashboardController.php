...
class DashboardController extends Controller
{
    private const WORKSPACE_ROUTES = [
        ...
        'production' => [
            ...
            'pointeur' => ['code' => 527194, 'route' => 'pointeur.workspace'],//ok
        ...
        'vente' => [
            'vendeur_boulangerie' => ['code' => 748596, 'route' => 'serveur.workspace'],//ok
            'vendeur_patisserie' => ['code' => 983214, 'route' => 'serveur.workspace']//ok
        ],
        'Informatique'=>[
            'developper' => ['code' => 7777, 'route' => 'admin.index']
        ]
    ...
    public function redirectToWorkspace()
    {
        ...
        if ($user->secteur === 'administration' && $user->role === 'dg') {
         ...
         // Vérification spéciale pour tous les employes
         if ($user->role != 'pdg') {
            $config2 = FirstConfigEmployee::where('user_id',$user->id)->first();
            
            if (!$config2) {
                Log::info("configuration non trouver creation..");
                FirstConfigEmployee::create([
                    'user_id' => Auth::id(),
                    'status' => false,
                ]);
                return redirect()->route('setup2.create');
            }else if($config2 && $config2->status === false) {
                Log::info("configuration trouver et status faux..");
                return redirect()->route('setup2.create');
            }else{
                Log::info("configuration trouver et status vrai");
                $t = 1;
            }
        }
