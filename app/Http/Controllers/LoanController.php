...
class LoanController extends Controller
{
    ...
    { 
        ...
        // Historiser l'action
        $this->historiser("L'utilisateur " . $employee->name . " a soumis une demande de prêt de {$validated['montant']} FCFA", 'request_loan');
    ...
    public function approveLoan(Request $request, $id)
    {
        ...
        // Historiser l'action
        $this->historiser("L'utilisateur " . $user->name . " a approuvé la demande de prêt de {$loan->amount} FCFA pour l'utilisateur ID: {$loan->user_id}", 'approve_loan');
    ...
    public function rejectLoan(Request $request, $id)
    {
        ...
        // Historiser l'action
        $this->historiser("L'utilisateur " . $user->name . " a refusé la demande de prêt ID: {$id}", 'reject_loan');
    ...
    public function setMonthlyRepayment(Request $request, $id)
    {
        ...
        // Vérifier si le montant de remboursement est negatif
        if ($validated['remboursement'] <= 0) {
            return redirect()->back()->with('error', 'Le montant de remboursement doit être supérieur à zéro.');
        }
        //verifier si l'employer a deja un remboursement non solder
        $existing_repayement = ACouper::where('id_employe', $id)
            ->where('remboursement', '>', 0)
            ->first();
        if ($existing_repayement) {
            return redirect()->back()->with('error', 'Cet employé a déjà un remboursement non soldé.');
        }
        ...
        // Historiser l'action
        $this->historiser("L'utilisateur " . $user->name . " a défini le montant de remboursement mensuel de {$validated['remboursement']} FCFA pour l'utilisateur ID: {$id}", 'set_monthly_repayment');
    ...
    public function recordRepayment(Request $request, $id)
    {
        ...
        $this->historiser("L'utilisateur " . $user->name . " a enregistré un remboursement de {$validated['montant']} FCFA pour l'utilisateur ID: {$id}", 'record_repayment');
