...
class AssignationMatiereController extends Controller
{
    ...
    {
        $producteurs = User::whereIn('role', ['boulanger', 'patissier','glace'])->get();
    ...
    public function store(Request $request)
    {
        ...
        // Si le délai n'est pas défini, le faire manuellement
        ...
        foreach ($request->matieres as $index => $matiereData) {
            ...
            // Conversion sécurisée pour le logging
            $uniteMinimaleString = ($uniteMinimale instanceof \App\Enums\UniteMinimale) 
                ? $uniteMinimale->value 
                : (string)$uniteMinimale;
    
            Log::info("Vérification de compatibilité pour la matière: {$matiere->nom}");
            Log::info("Unité assignée: {$uniteAssignee}, Unité minimale: {$uniteMinimaleString}");
    
            // Vérifier si les unités sont compatibles - avec des logs détaillés
            try {
                [$estCompatible, $messageErreur] = $this->conversionService->verifierCompatibilite($uniteAssignee, $uniteMinimale);
    
                if (!$estCompatible) {
                    Log::error("Incompatibilité d'unités: {$messageErreur}");
                ...
                Log::error("Exception lors de la vérification de compatibilité: " . $e->getMessage());
                Log::error("Trace: " . $e->getTraceAsString());
                return redirect()->back()
                    ->withInput()
                    ->withErrors([
                        "matieres.{$index}.unite" => "Erreur lors de la vérification de compatibilité: " . $e->getMessage()
                    ]);
            }
        ...
        try {
            ...
            foreach ($request->matieres as $matiereData) {
                ...
                ->whereDate('created_at', Carbon::today()) // Ajout de cette condition
                ->orderBy('created_at', 'desc')
                ->first();
            ...
            // Historiser l'action
            $actionDetails = "Assignation de matières à {$producteur->name} par " . Auth::user()->name . ": ";
    ...
    public function update(Request $request, $id)
    {
        ...
        if($assignation->date_limite_utilisation < Carbon::now()) {
            return redirect()->back()
                ->withInput()
                ->withErrors(['date_limite' => 'Impossible de modifier une assignation expirée.']);
        }
        ...
        //historiser l'action en precisant les détails de l'assignation
            $details = "Assignation de matières {$assignation->id} mise à jour: ";
            $details .= "{$request->quantite} {$request->unite} pour {$matiere->nom}.";
            if ($request->filled('date_limite')) {
                $details .= " Date limite: {$request->date_limite}.";
            }
            
            // Historiser l'action
            $this->historiser($details, 'update_assignation');
    ...
    public function mesAssignations()
    {
        ...
        return view('assignations.mes-assignations', compact('secteur','nom','assignations','reservations'));
    }
