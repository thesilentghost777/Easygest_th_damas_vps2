...
class DeliController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // Historiser l'action
            $this->historiser("Enregistrement d'un incident '{$validated['nom']}' d'un montant de {$validated['montant']} FCFA par {$user->name}", 'create_deli');
    ...
    public function update(Request $request, Deli $deli)
{
    ...
    // Mettre la date à défaut à aujourd'hui
    ...
    if (!empty($validated['employes'])) {
        // Préparer les données avec date_incident pour chaque employé
        $employesData = [];
        foreach ($validated['employes'] as $employeId) {
            $employesData[$employeId] = ['date_incident' => $validated['date_incident']];
        }
        
        $deli->employes()->sync($employesData);
    } else {
        $deli->employes()->detach();
    }

    //historiser l'action
    $this->historiser("Mise à jour de l'incident '{$deli->nom}' par " . Auth::user()->name, 'update_deli');
    return redirect()->route('delis.index')
    ...
    public function destroy(Deli $deli)
    {
        ...
        $deli->delete();
        //historiser l'action
        $this->historiser("Suppression de l'incident '{$deli->nom}' par " . Auth::user()->name, 'delete_deli');
        return redirect()->route('delis.index')
