...
class HoraireController extends Controller
{
    ...
    public function marquerArrivee()
    {
        $user = auth()->user();
        $today = Carbon::today();
        // Vérifier s'il existe déjà un enregistrement pour aujourd'hui et cet utilisateur
        $debutJournee = $today->copy()->startOfDay(); // 00:00:00
        $finJournee = $today->copy()->endOfDay();     // 23:59:59
        
        $horaireExistant = Horaire::where('employe', $user->id)
            ->whereBetween('arrive', [$debutJournee, $finJournee])
            ->first();
    ...
    public function enregistrerHoraire(Request $request)
    {
        ...
        if ($horaireExistant) {
            ...
            $horaireExistant->update([
                'arrive' => $arrive,
                'depart' => $depart
            ]);
            $horaire = $horaireExistant;
            $action = 'mis à jour';
        } else {
        ...
        // Historisation de l'action
        $this->historiser("L'utilisateur {$user->name} a {$action} manuellement ses horaires: arrivée à {$horaire->arrive->format('H:i')}, départ à {$horaire->depart->format('H:i')}", 'enregistrement_manuel');
    
        return redirect()->back()->with('success', "Horaires {$action}s avec succès");
    }
