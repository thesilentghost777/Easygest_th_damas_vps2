...
class ExtraController extends Controller
{
    ...
    public function index2()
    {
        ...
        if($user->secteur !== 'administration') {
            $extras = Extra::where('secteur', $user->secteur)->orderBy('secteur')->paginate(10);
        } else {
            $extras = Extra::orderBy('secteur')->paginate(10);
        }
    ...
    public function update(Request $request, Extra $extra)
    {
        ...
        // Historiser l'action
        $user = auth()->user();
        $this->historiser("L'utilisateur {$user->name} a modifié le reglement {$extra->nom}", 'update_extra');
        return redirect()->route('extras.index')
    ...
    public function destroy(Extra $extra)
    {
        ...
        // Historiser l'action
        $user = auth()->user();
        $this->historiser("L'utilisateur {$user->name} a supprimé le reglement {$extra->nom}", 'delete_extra');
        return redirect()->route('extras.index')
