...

{
    ...
    public function create()
    {
        ...
        $servers = User::where('secteur', 'vente')
        ->orWhere('role', 'glace')
...

    {
        ...
        $this->historiser('Création d\'une assignation de sacs par '.Auth()->user()->name.' pour le sac '.$bag->id.' à l\'utilisateur '.$validated['user_id'].' quantité '.$validated['quantity_assigned'], 'bag_assignments_create');
...

    {
        ...
        // Enregistrer l'historique de l'action
        $this->historiser('Modification de l\'assignation de sacs par '.Auth()->user()->name.' pour le sac '.$bag->id.' à l\'utilisateur '.$assignment->user_id.' de la quantité '.$oldQuantity.' à '.$newQuantity, 'bag_assignments_update');
