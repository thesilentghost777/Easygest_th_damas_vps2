...
class RecetteController extends Controller
{
    ...
    public function create()
    {
       ...
       $matiere = Matiere::where('nom', 'not like', 'Taule%')
            ->where('nom', 'not like', 'produit Avari%')
            ->get();
    ...
    public function store(Request $request)
    {
        try {
            ...
            $this->historiser("Recette ajoutée pour le produit {$request->produit} avec les matières: " . json_encode($request->matieres), 'add_recipe');
            DB::commit();
    ...
    public function destroy($produitId)
{
    try {
        ...
        //historiser l'action en specifiant les details important
        $this->historiser("Recette supprimée pour le produit {$produitId}", 'delete_recipe');
        DB::commit();
