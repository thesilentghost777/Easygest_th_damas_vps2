...
class BagRecoveryController extends Controller
{
    ...
    public function recover(Request $request, BagSale $sale)
    {
        ...
        $sale->save();
        // Historiser l'action
        $this->historiser('Récupération de sacs invendus par '.Auth()->user()->name.' pour la vente '.$sale->id.' du sac '.$bag->id.' de la quantité '.$validated['quantity_to_recover'], 'bag_recovery_recover');
