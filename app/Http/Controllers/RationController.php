...
class RationController extends Controller
{
    ...
    public function updateDefaultRation(Request $request)
    {
        ...
        $this->historiser("Montant de ration par défaut mis à jour à {$request->montant_defaut}", 'update_default_ration');
    ...
    public function updateEmployeeRation(Request $request)
    {
        ...
        $employeeRation->personnalise = true;
        $employeeRation->save();
        //historiser l'action en specifiant les details important
        $this->historiser("Ration de l'employé {$employeeRation->employee->name} mise à jour avec un montant de {$request->montant}", 'update_employee_ration');
    ...
    public function claim(Request $request)
    {
        ...
        if (!$horaire) {
            return redirect()->route('rations.employee.claim')->with('error', 'Vous n\'êtes pas marqué comme présent aujourd\'hui');
        }
        ...
        if ($alreadyClaimed) {
            return redirect()->route('rations.employee.claim')->with('error', 'Vous avez déjà réclamé votre ration pour aujourd\'hui');
        }
        ...
        //historiser l'action en specifiant les details important
        $this->historiser("L'employé {$employee->name} a réclamé sa ration de {$montantRation} pour le {$today->toDateString()}", 'claim_ration');
        return redirect()->route('rations.employee.claim')->with('success', 'Ration réclamée avec succès');
    }
}
