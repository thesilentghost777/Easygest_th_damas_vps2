...
class TauleInutiliseeController extends Controller
{
    ...
    public function recuperer(TauleInutilisee $tauleInutilisee)
    {
        ...
        try {
            ...
            // Historiser l'action
            $this->historiser("Récupération des taules inutilisées: {$tauleInutilisee->typeTaule->nom} - {$tauleInutilisee->nombre_taules} par " . Auth::user()->name, 'recuperation_taules_inutilisees');
            DB::commit();
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // Historiser l'action
            $this->historiser("Création de taules inutilisées: {$typeTaule->nom} - {$nombreTaules} par " . Auth::user()->name, 'creation_taules_inutilisees');
            DB::commit();
