...
class StockController extends Controller
{
    ...
    public function index()
    {
        ...
        ->select(
            ...
            'Produit_fixes.prix', // Ajoutez cette ligne
        ...
        $valeur_stock_produits = $produits->sum(function($produit) {
            return $produit->quantite_totale * ($produit->prix ?? 0);
        });
    ...
    public function adjustProduitQuantity(Request $request, $produit)
    {
        ...
        // Historiser l'action
        $this->historiser("Ajustement de la quantité du produit: {$produit} - {$validated['operation']} de {$adjustmentQuantity}", 'ajustement_produit');
    ...
    public function updateMatiere(Request $request, Matiere $matiere)
    {
        ...
        // Historiser l'action
        $this->historiser("Mise à jour de la matière: {$matiere->nom}", 'mise_a_jour_matiere');
        return response()->json(['success' => true]);
    }
    ...
    public function deleteMatiere(Matiere $matiere)
    {
        $matiere->delete();
        // Historiser l'action
        $this->historiser("Suppression de la matière: {$matiere->nom}", 'suppression_matiere');
        return response()->json(['success' => true]);
    }
    ...
    public function deleteProduit($produit)
    {
        ...
        // Historiser l'action
        $this->historiser("Suppression du produit: {$produit}", 'suppression_produit');
        return response()->json(['success' => true]);
    }
    ...
    public function adjustMatiereQuantity(Request $request, Matiere $matiere)
    {
        ...
        // Historiser l'action
        $this->historiser("Ajustement de la quantité de la matière: {$matiere->nom} - {$validated['operation']} de {$validated['quantite']}", 'ajustement_matiere');
        return response()->json(['success' => true]);
    }
    ...
    public function updateProduit(Request $request, $produit)
    {
        ...
        // Historiser l'action
        $this->historiser("Mise à jour du produit: {$produit} avec quantite {$validated['quantite']} et prix {$validated['prix']}", 'mise_a_jour_produit');
        return response()->json(['success' => true]);
    }
