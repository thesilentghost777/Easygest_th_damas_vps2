...
class AvarieController extends Controller
{
    ...
    public function create()
    {
        ...
        #en fonction du role de l'utilisateur, on affiche les produits disponibles
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $this->historiser(
                "Stock d'avaries mis à jour par " . Auth::user()->name . " pour le produit #{$validated['produit']} : +{$validated['quantite_produit']} unités",
                'update',
                $stock->id,
                'produit_stock'
            );
    ...
    {
       // Récupérer les avaries de production (utilisations avec quantité_produit négative)
       // Récupérer les avaries de production depuis la table produit_stocks
    $avariesProduction = DB::table('produit_stocks as ps')
    ->join('Produit_fixes as pf', 'ps.id_produit', '=', 'pf.code_produit')
    ->select(
        'ps.id',
        'pf.nom as nom_produit',
        'pf.prix as prix_produit',
        'ps.quantite_avarie',
        'ps.updated_at as date_avarie'
    )
    ->where('ps.quantite_avarie', '>', 0)
    ->orderBy('ps.updated_at', 'desc')
    ->get()
    ->map(function ($avarie) {
        $avarie->valeur_avarie = $avarie->quantite_avarie * $avarie->prix_produit;
        return $avarie;
    });

   // Récupérer les avaries de vente (transactions avec type 'Produit Avarier')
   $avariesVente = DB::table('transaction_ventes as tv')
       ->join('Produit_fixes as pf', 'tv.produit', '=', 'pf.code_produit')
       ->join('users as serveur', 'tv.serveur', '=', 'serveur.id')
       ->select(
           'tv.id',
           'pf.nom as nom_produit',
           'tv.quantite',
           'tv.prix',
           'serveur.name as nom_serveur',
           'tv.date_vente as date_avarie',
           'tv.monnaie'
       )
       ->where('tv.type', 'Produit Avarie')
       ->orderBy('tv.date_vente', 'desc')
       ->get()
       ->map(function ($avarie) {
           $avarie->valeur_avarie = $avarie->quantite * $avarie->prix;
           return $avarie;
       });

   // Calculer les totaux
   $totalAvariesProduction = $avariesProduction->sum('valeur_avarie');
   $totalAvariesVente = $avariesVente->sum('valeur_avarie');

   return view('producteur.avaries.index', compact(
       'avariesProduction',
       'avariesVente',
       'totalAvariesProduction',
       'totalAvariesVente'
   ));
    }
     ...
     protected function deduireQuantiteAssignee($producteurId, $matiereId, $quantite)
{
    ...
    $assignation = AssignationMatiere::where('producteur_id', $producteurId)
        ...
        ->where('date_limite_utilisation', '>=', now())
        ->first();
