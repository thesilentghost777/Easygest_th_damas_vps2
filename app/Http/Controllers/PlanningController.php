...
class PlanningController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Historiser l'action
        $this->historiser("L'utilisateur " . auth()->user()->name . " a créé un nouveau planning pour l'employé ID: {$planning->employe}", 'create_planning');
        return response()->json([
            'status' => 'success',
            'planning' => $planning
        ]);
    ...
    public function update(Request $request, Planning $planning)
    {
        ...
        // Historiser l'action
        $this->historiser("L'utilisateur " . auth()->user()->name . " a mis à jour le planning ID: {$planning->id}", 'update_planning');
        return response()->json([
            'status' => 'success',
            'planning' => $planning
        ]);
    ...
    public function destroy(Planning $planning)
    {
        $planning->delete();
        // Historiser l'action
        $this->historiser("L'utilisateur " . auth()->user()->name . " a supprimé le planning ID: {$planning->id}", 'delete_planning');
        return response()->json([
            'status' => 'success'
        ]);
    }
