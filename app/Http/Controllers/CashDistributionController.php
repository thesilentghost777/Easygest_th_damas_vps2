...
class CashDistributionController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $user = auth()->user();
        if ($user->secteur == 'vente' || $user->secteur == 'glace') {
            // Si l'utilisateur n'est pas du secteur vente, ne montrer que ses propres distributions
        ...
        $sellers = User::where('secteur', 'vente')
        ->orWhere('secteur', 'glace')
        ->get();



        return view('cash.distributions.index', compact('distributions', 'sellers', 'flag'))
    ...
    public function store(Request $request)
    {
        ...
        if ($user->secteur !== 'vente' && $user->secteur !== 'glace') {
            return redirect()->route('cash.distributions.create')
                            ->with('error', 'Seuls les utilisateurs du secteur vente ou glace peuvent créer des distributions de monnaie.')
                            ->withInput();
        }
        ...
        // Historiser l'action
        $this->historiser('Création de distribution de monnaie par '.Auth()->user()->name.' pour la vendeuse '.$user->name.' à la date '.$validated['date'].' avec le montant des billets '.$validated['bill_amount'].' et le montant initial de pièces '.$validated['initial_coin_amount'], 'cash_distributions_create');
        return redirect()->route('cash.distributions.index')
    ...
    public function show(CashDistribution $distribution)
    {
        ...
        return view('cash.distributions.show', compact('distribution', 'sales', 'bagSalesAmount', 'flag'))
    ...
    public function update(Request $request, CashDistribution $distribution)
    {
        ...
        //historiser l'action
        $this->historiser('Mise à jour de la distribution de monnaie par '.Auth()->user()->name.' pour la vendeuse '.$distribution->user->name.' à la date '.$distribution->date.' avec le montant des billets '.$validated['bill_amount'].' et le montant initial de pièces '.$validated['initial_coin_amount'], 'cash_distributions_update');
    ...
    public function close(Request $request, CashDistribution $distribution)
{
    ...
    try {
        ...
        $user = Auth::user();
        $distribution->calculateMissingAmount($user);
        ...
        $distribution->save();
        //historiser l'action
        $this->historiser('Clôture de la distribution de monnaie par '.Auth()->user()->name.' pour la vendeuse '.$distribution->user->name.' à la date '.$distribution->date.' avec le montant final de pièces '.$validated['final_coin_amount'].' et le montant déposé '.$validated['deposited_amount'], 'cash_distributions_close');
        \DB::commit();
