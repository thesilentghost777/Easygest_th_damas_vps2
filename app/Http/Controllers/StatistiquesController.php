...

{
    ...
    public function horaires()
    {
        ...
        // Valeur par défaut si $tauxPonctualite est null
        if ($tauxPonctualite->isEmpty()) {
            $tauxPonctualite = collect([
                (object)[
                    'id' => 0,
                    'name' => 'Aucun employé',
                    'secteur' => 'Non défini',
        ...
        // Valeur par défaut si $absenteismeParSecteur est null
        if ($absenteismeParSecteur->isEmpty()) {
            $absenteismeParSecteur = collect([
                (object)[
                    'secteur' => 'Non défini',
                    'total_employes' => 0,
                    'employes_absents' => 0,
                    'taux_absenteisme' => 0.00
                ]
            ]);
        }
        ...
        // Valeur par défaut si $plusAbsentParSecteur est null ou vide
        if ($plusAbsentParSecteur->isEmpty()) {
            $plusAbsentParSecteur = collect([
                'Non défini' => (object)[
                    'secteur' => 'Non défini',
                    'name' => 'Aucun employé',
                    'nombre_absences' => 0
        ...
        // Valeur par défaut si $plusPonctuel est null
        if (!$plusPonctuel) {
            $plusPonctuel = (object)[
                'name' => 'Aucun employé',
                'taux_ponctualite' => 0.00
        ...
        // Valeur par défaut si $statistiquesHoraires est null
        if ($statistiquesHoraires->isEmpty()) {
            $statistiquesHoraires = collect([
                (object)[
                    'name' => 'Aucun employé',
                    'arrive_plus_tot' => '08:00:00',
                    'depart_plus_tard' => '17:00:00',
                    'heure_arrivee_moyenne' => 8,
                    'heure_depart_moyenne' => 17,
        ...
        // Valeur par défaut si $heuresSupp est null
        if ($heuresSupp->isEmpty()) {
            $heuresSupp = collect([
                (object)[
                    'name' => 'Aucun employé',
                    'heures_supp_total' => 0
        ...
        // Valeur par défaut si $tauxConges est null
        if ($tauxConges->isEmpty()) {
            $tauxConges = collect([
                (object)[
                    'secteur' => 'Non défini',
                    'name' => 'Aucun employé',
                    'jours_conges' => 0,
                    'taux_utilisation_conges' => 0.00
                ]
            ]);
        }
        ...
        // Valeur par défaut si $statistiquesGlobales est null
        if ($statistiquesGlobales->isEmpty()) {
            $statistiquesGlobales = collect([
                (object)[
                    'name' => 'Aucun employé',
                    'jours_travailles' => 0,
        ...
        // Valeur par défaut si $variationHoraires est null
        if ($variationHoraires->isEmpty()) {
            $variationHoraires = collect([
                (object)[
                    'name' => 'Aucun employé',
                    'variation_arrivee' => 0,
                    'variation_depart' => 0
        ...
        // Valeur par défaut si $absenceRatePerEmployee est null
        if ($absenceRatePerEmployee->isEmpty()) {
            $absenceRatePerEmployee = collect([
                (object)[
                    'name' => 'Aucun employé',
                    'days_present' => 0,
                    'days_absent' => 0
        ...
        // Valeur par défaut si $workTimePerDay est null
        if ($workTimePerDay->isEmpty()) {
            $workTimePerDay = collect([
                (object)[
        ...
        // Valeur par défaut si $monthlyWorkEvolution est null
        if ($monthlyWorkEvolution->isEmpty()) {
            $monthlyWorkEvolution = collect([
                (object)[
                    'name' => 'Aucun employé',
                    'month' => now()->month,
                    'year' => now()->year,
        ...
        // Valeur par défaut si $monthlyAbsenceEvolution est null
        if ($monthlyAbsenceEvolution->isEmpty()) {
            $monthlyAbsenceEvolution = collect([]);
            // Créer des valeurs par défaut pour chaque mois de l'année
            for ($i = 1; $i <= 12; $i++) {
                $monthlyAbsenceEvolution->push((object)[
                    'month' => $i,
                    'absent_count' => 0
                ]);
            }
    ...
    public function ventes()
    {
        ...
        $chiffreAffaires = [
            'journalier' => TransactionVente::where('type','Vente')->whereDate('date_vente', Carbon::today())
                ->sum(DB::raw('prix * quantite')),
            'hebdomadaire' => TransactionVente::where('type','Vente')->whereBetween('date_vente',
                [Carbon::now()->startOfWeek(), Carbon::now()->endOfWeek()])
                ->sum(DB::raw('prix * quantite')),
            'mensuel' => TransactionVente::where('type','Vente')->whereMonth('date_vente', Carbon::now()->month)
                ->sum(DB::raw('prix * quantite'))
        ];

        $produitsPopulaires = TransactionVente::where('type', 'Vente')
            ->join('Produit_fixes', 'transaction_ventes.produit', '=', 'Produit_fixes.code_produit')
            ->select('Produit_fixes.nom as nom_produit',
                DB::raw('SUM(transaction_ventes.quantite * Produit_fixes.prix) as total_vendu'))
        ...
        // 3. État des stocks
        $stocks = ProduitStock::with('produitFixe')
        ...
        // 5. Versements
        $versements = [
            'total' => VersementChef::join('users', 'Versement_chef.verseur', '=', 'users.id') // Jointure avec la table des utilisateurs
                ->where('Versement_chef.status', true) // Filtre par statut validé
                ->where('users.secteur', 'vente') // Filtre par secteur du verseur
                ->orWhere('users.secteur', 'glace')
                ->sum('Versement_chef.montant'), // Calcul du total des sommes

            'par_status' => VersementChef::select('Versement_chef.status', DB::raw('COUNT(*) as total'))
                ->join('users', 'Versement_chef.verseur', '=', 'users.id') // Jointure avec la table des utilisateurs
                ->where('users.secteur', 'vente') // Filtre par secteur du verseur
                ->groupBy('Versement_chef.status')
                ->get()
        ];
        ...
        $performanceServeurs = TransactionVente::where('type','Vente')->select('serveur',
        ...
        $evolutionVentes = TransactionVente::where('type','Vente')
        ->select(
        ...
        // 8. Analyse paiements
        $methodePaiement = TransactionVente::where('type','Vente')->select('monnaie',
        ...
        $statsParPeriode = [
            'mois' => TransactionVente::where('type','Vente')->select(
                ...
                ->limit(5)
                ->get(),
            'jours' => TransactionVente::where('type','Vente')->select(
...
->get();
        ...
        $coutMatieresPremiere = 0;

        // 5. Évolution par année
        $evolutionAnnuelle = TransactionVente::where('type', 'Vente')->select(
        ...
        $top5Mois = TransactionVente::where('type', 'Vente')->select(
            DB::raw('YEAR(date_vente) as annee'),
            DB::raw('MONTH(date_vente) as mois'),
            DB::raw('SUM(prix * quantite) as total'),
            DB::raw('COUNT(*) as nombre_ventes'),
        ...
        $evolutionVersements = VersementChef::select(
        ...
        $topProduitsAvaries = DB::table('transaction_ventes')
    ->join('Produit_fixes', 'transaction_ventes.produit', '=', 'Produit_fixes.code_produit')
    ->select(
        'Produit_fixes.nom',
        DB::raw('SUM(CASE WHEN transaction_ventes.type = "Produit Avarie" THEN transaction_ventes.quantite ELSE 0 END) as quantite_avarie'),
        'Produit_fixes.prix',
        DB::raw('(SUM(CASE WHEN transaction_ventes.type = "Produit Avarie" THEN transaction_ventes.quantite ELSE 0 END) * 100.0 / 
                 SUM(transaction_ventes.quantite)) as pourcentage_avarie'),
        DB::raw('(SUM(CASE WHEN transaction_ventes.type = "Produit Avarie" THEN transaction_ventes.quantite ELSE 0 END) * Produit_fixes.prix) as prix_total_avarie')
    )
    ->groupBy('transaction_ventes.produit', 'Produit_fixes.nom', 'Produit_fixes.prix')
    ->having('quantite_avarie', '>', 0)
