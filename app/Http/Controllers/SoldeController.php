...

{
...

    {
        ...
        $remboursementTotalesMois = Acouper::whereMonth('date', now()->month)
        ...
        $remboursementTotalesMois = Acouper::whereMonth('date', now()->month)
                ->whereYear('date', now()->year)
                ->sum('remboursement');
        // MontaSalairent total à prévoir pour les salaires

        $montantPrevisionnel = $totalSalaires + $totalPrimes - $totalManquants - $totalDelis - $totalAvances - $remboursementTotalesMois;
        ...
        $montantCaisseSociale = $caisseSociale;
        $nombre_employe = User::count();
        $montantCaisseSocialetotal = $montantCaisseSociale*$nombre_employe;
        $montantPrevisionnel-=$montantCaisseSocialetotal;
        // Montant pour les enveloppes (montant prévisionnel - caisse sociale)

        $montantEnveloppes = $montantPrevisionnel;
