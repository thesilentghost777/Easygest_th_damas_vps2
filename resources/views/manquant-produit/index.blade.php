    ...
    <div class="container">
        ...
        <div class="mb-6">
            ...
            <h1 class="page-title font-bold text-gray-800">
                {{ $isFrench ? 'Calcul des Manquants - Produits Finis' : 'Shortage Calculation - Finished Products' }}
            </h1>
            <p class="text-center text-gray-600 mb-6">
                {{ $isFrench ? 'Saisissez les quantités réelles pour calculer les manquants' : 'Enter actual quantities to calculate shortages' }}
            </p>
        </div>

        <div class="mobile-card">
            <form action="{{ route('manquant-produit.calculer') }}" method="POST" id="produitForm">
                @csrf
                    ...
                    @foreach($produits as $produit)
                        <div class="produit-card">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                <div>
                                    ...
                                    <div class="quantity-display">
                                        {{ number_format($produit->stock ? $produit->stock->quantite_en_stock : 0) }} {{ $isFrench ? 'unités' : 'units' }}
                                    </div>
                                </div>
                                
                                <div>
                                    <label for="quantite_{{ $produit->code_produit }}" class="form-label">
                                        {{ $isFrench ? 'Quantité Réelle' : 'Actual Quantity' }}
                                    </label>
                                    <input 
                                        type="number" 
                                        name="quantites_reelles[{{ $produit->code_produit }}]" 
                                        id="quantite_{{ $produit->code_produit }}"
                                        class="form-input" 
                                        min="0"
                                        placeholder="0"
                                        required
                            ...
                            <div class="text-xs text-gray-500 text-center">
                                {{ $isFrench ? 'Prix unitaire' : 'Unit price' }}: {{ number_format($produit->prix) }} XAF
                            </div>
                        </div>
                    @endforeach
                </div>
