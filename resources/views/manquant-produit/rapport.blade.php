    ...
    <div class="container">
        ...
        <div class="print-header">
            ...
            <h1 class="page-title font-bold text-gray-800">
                {{ $isFrench ? 'Rapport des Manquants - Produits Finis' : 'Shortage Report - Finished Products' }}
            </h1>
        ...
        <div class="no-print mb-6">
            <div class="actions-grid">
                ...
                <a href="{{ route('manquant-produit.index') }}" class="action-btn">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="summary-card">
            ...
            <p class="text-center text-amber-600 mt-2">
                {{ count($manquants) }} {{ $isFrench ? 'produit(s) manquant(s)' : 'missing product(s)' }}
            </p>
        </div>
        @endif
                    ...
                    <table class="min-w-full bg-white border border-gray-200 rounded-lg">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                    {{ $isFrench ? 'Qté en Stock' : 'Stock Qty' }}
                                </th>
                                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-red-50">
                                <td class="px-4 py-3 text-sm font-medium text-gray-900">
                                    {{ $manquant['produit']->nom }}
                                </td>
                                <td class="px-4 py-3 text-sm text-gray-700">
                                    <span class="category-badge">{{ $manquant['produit']->categorie }}</span>
                                </td>
                                <td class="px-4 py-3 text-sm text-gray-700">
                                    {{ number_format($manquant['quantite_attendue']) }}
                                </td>
                                <td class="px-4 py-3 text-sm text-gray-700">
                                    {{ number_format($manquant['quantite_reelle']) }}
                                </td>
                                <td class="px-4 py-3 text-sm font-bold text-red-600">
                                    {{ number_format($manquant['manquant']) }}
                                </td>
                                <td class="px-4 py-3 text-sm font-bold text-red-600">
                ...
                <div class="mobile-cards">
                    ...
                    <div class="manquant-item">
                        <div class="flex justify-between items-start mb-2">
                            <h4 class="font-bold text-red-800 text-lg">{{ $manquant['produit']->nom }}</h4>
                            <span class="category-badge">{{ $manquant['produit']->categorie }}</span>
                        </div>
                        
                        <div class="grid grid-cols-2 gap-3 mb-3">
                            <div class="text-center">
                                <p class="text-xs text-gray-600">{{ $isFrench ? 'En Stock' : 'In Stock' }}</p>
                                <p class="font-semibold">{{ number_format($manquant['quantite_attendue']) }}</p>
                            </div>
                            <div class="text-center">
                                <p class="text-xs text-gray-600">{{ $isFrench ? 'Réelle' : 'Actual' }}</p>
                                <p class="font-semibold">{{ number_format($manquant['quantite_reelle']) }}</p>
                            </div>
                        </div>
                        ...
                        <div class="bg-red-100 p-3 rounded-lg text-center">
                            ...
                            <p class="font-bold text-red-800 text-lg">
                                {{ number_format($manquant['manquant']) }} {{ $isFrench ? 'unités' : 'units' }}
                            </p>
            ...
            @else
                <div class="text-center py-8">
                    ...
                    <p class="text-green-600">
                        {{ $isFrench ? 'Tous les produits finis sont en stock suffisant.' : 'All finished products are in sufficient stock.' }}
                    </p>
                </div>
            @endif
        </div>
