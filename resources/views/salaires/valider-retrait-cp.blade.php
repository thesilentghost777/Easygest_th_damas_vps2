                ...
                @else
                    ...
                    <div class="lg:hidden space-y-4">
                        ...
                        <div class="bg-white border-2 border-gray-200 rounded-xl p-4 shadow-sm transform transition-all duration-200 active:scale-98">
                                ...
                                <div class="flex items-center space-x-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-gray-600 font-medium">
                                            {{ $isFrench ? "Montant" : "Amount" }}: <span class="text-green-600 font-bold">{{ number_format($demande->sommeAs, 0, ',', ' ') }} XAF</span>
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            @else
                                <button type="button" onclick="openPinModal({{ $demande->id }})" class="w-full bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white font-medium py-3 px-6 rounded-xl border-2 border-blue-600 transition-all duration-200 active:scale-95 flex items-center justify-center shadow-lg">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                                    </svg>
                                    {{ $isFrench ? "Valider le retrait" : "Validate withdrawal" }}
                        ...
                        @foreach($demandes as $demande)
                            <div class="border-2 border-gray-200 rounded-none p-4 md:p-6 hover:shadow-md transition-all duration-200">
                                <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                                    <div>
                                        <h3 class="text-lg font-semibold text-blue-700">{{ $demande->employe->name }}</h3>
                                        <p class="text-gray-600 font-medium mt-1">{{ $isFrench ? "Montant" : "Amount" }}: <span class="text-green-600 font-bold">{{ number_format($demande->sommeAs, 0, ',', ' ') }} XAF</span></p>
                                    </div>
                                    ...
                                    @else
                                        <button type="button" onclick="openPinModal({{ $demande->id }})" class="w-full md:w-auto bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-6 rounded-none border-2 border-blue-600 transition-colors duration-200 flex items-center justify-center">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                                            </svg>
                                            {{ $isFrench ? "Valider le retrait" : "Validate withdrawal" }}
...
<div id="pinModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden transition-opacity duration-300 opacity-0">
    <div class="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-md transform transition-all duration-300 scale-95 mx-4">
        ...
        <form id="pinForm" action="{{ route('recup-retrait-cp') }}" method="POST">
            @csrf
