...
<div class="container mx-auto px-4 py-8 max-w-4xl">
    <div class="mb-8">
        ...
        <p class="text-gray-600">
            {{ $isFrench ? 'Enregistrez les produits fabriqués dans un sac et suivez la performance.' : 'Record products manufactured in a bag and track performance.' }}
        </p>
    </div>

    <div class="bg-white rounded-lg shadow-lg p-6">
        <form action="{{ route('boulangerie.production.store') }}" method="POST" id="productionForm">
            @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="date" 
                               ...
                               value="{{ old('date_production', date('Y-m-d')) }}"
                               required
            ...
            <div class="mb-8">
                ...
                <textarea id="observations" 
                          ...
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('observations') border-red-500 @enderror">{{ old('observations') }}</textarea>
                @error('observations')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex justify-end space-x-4">
                ...
                <button type="submit" 
                    ...
                    {{ $isFrench ? 'Enregistrer la production' : 'Save Production' }}
                </button>
            </div>
        </form>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.updateProduitPrix = function(selectElement, index) {
        ...
        const quantiteInput = selectElement.closest('.bg-gray-50').querySelector('input[type="number"]');
        updateTotal(index);
    };
